...
func ScaleUp(context *context.AutoscalingContext, processors *ca_processors.AutoscalingProcessors, clusterStateRegistry *clusterstate.ClusterStateRegistry, unschedulablePods []*apiv1.Pod,
	...
	if bestOption != nil && bestOption.NodeCount > 0 {
		...
		if context.MaxNodesPerScaleUp > 0 && newNodes > context.MaxNodesPerScaleUp {
			klog.V(1).Infof("Estimated nodes %d exceeds max nodes per scale up event of %d; setting estimated nodes to %d", newNodes, context.MaxNodesPerScaleUp, context.MaxNodesPerScaleUp)
